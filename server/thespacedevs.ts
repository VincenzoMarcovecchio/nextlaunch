...
import {
	updateLaunchLibraryv2DBAgencies,
	updateLaunchLibraryv2DBLaunches,
	updateLaunchLibraryv2DBLaunchers,
} from "./database";

const LAUNCH_ALL_URL =
	"https://ll.thespacedevs.com/2.0.0/launch/?format=json&mode=detailed&limit=100";

const LAUNCH_UPCOMING_URL =
	"https://ll.thespacedevs.com/2.0.0/launch/upcoming/?format=json&mode=detailed&limit=100";

const AGENCY_URL =
	"https://ll.thespacedevs.com/2.0.0/agencies/?format=json&mode=detailed&limit=100";

const LAUNCHER_URL =
	"https://ll.thespacedevs.com/2.0.0/config/launcher/?format=json&mode=detailed&limit=100";
...
export async function getLauncLibraryv2Launches(
	upcoming: boolean
): Promise<LaunchLibraryV2Launch[]> {
	let url = upcoming ? LAUNCH_UPCOMING_URL : LAUNCH_ALL_URL;

	return getLauncLibraryV2Rows<LaunchLibraryV2Launch>(url);
}

export async function getLauncLibraryv2Agencies(): Promise<
	LaunchLibraryV2Launch[]
> {
	return getLauncLibraryV2Rows<any>(AGENCY_URL);
}

export async function getLauncLibraryv2Launchers(): Promise<
	LaunchLibraryV2Launch[]
> {
